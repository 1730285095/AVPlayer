...
#import "W_ProgressSlider.h"
@implementation W_VideoModel
...
@end
@interface W_AVPlayerViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@property (strong, nonatomic) W_ProgressSlider *slider;
...
@property (strong, nonatomic) W_VideoModel *videoModel;
@property (strong, nonatomic) NSArray *videoArr;
@end

@implementation W_AVPlayerViewController
...
- (void)ac_initPlayer
{
    ...
    W_AVPlayerView *avPlayerView = [[W_AVPlayerView alloc] initWithMoviePlayerLayer:self.avPlayerLayer frame:self.view.bounds];
    ...
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapBlick:)];
...
- (void)ac_initSubViews
{
    ...
    //滑块
    self.slider = [[W_ProgressSlider alloc] initWithFrame:CGRectZero direction:AC_SliderDirectionHorizonal];
...
//切换当前播放的内容
- (void)changeCurrentplayerItemWithAC_VideoModel:(W_VideoModel *)model
{
    if (self.avPlayer) {
...
//处理滑块
- (void)progressValueChange:(W_ProgressSlider *)slider
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    W_VideoModel *model = self.videoArr[indexPath.row];
...
-(void)tapBlick:(UITapGestureRecognizer *)tapges{
    [self showOrHideBar];
}
